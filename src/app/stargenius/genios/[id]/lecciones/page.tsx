...
import { uploadFileStarGenius, generateStoragePathStarGenius, deleteFileByUrlStarGenius, isStorageUrlStarGenius } from '@/lib/supabase/stargenius-storage';
...
export default function LeccionesPage() {
  ...
  const fetchGenius = useCallback(async () => {
    const { data } = await supabaseStarGenius
      .from('geniuses')
      .select('name')
      .eq('id', geniusId)
      .single();
  ...
  useEffect(() => {
    const loadData = async () => {
      setIsLoading(true);
      await Promise.all([fetchGenius(), fetchLessons()]);
      setIsLoading(false);
    };
    loadData();
  }, [fetchGenius, fetchLessons]);
  ...
  const openEditModal = (lesson: Lesson) => {
    ...
    setForm({
      ...
      intelligence_type: lesson.intelligence_type || '',
    });
    setVideoFile(null);
    setThumbnailFile(null);
    setShowModal(true);
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      // Subir video
      const videoPath = generateStoragePathStarGenius('lessons', tempId, videoFile.name);
      const videoUrl = await uploadFileStarGenius(videoPath, videoFile);
      if (!videoUrl) throw new Error('No se pudo subir el video');

      // Subir thumbnail si existe
      let thumbnailUrl: string | null = null;
      if (thumbnailFile) {
        const thumbPath = generateStoragePathStarGenius('thumbnails', tempId, thumbnailFile.name);
        thumbnailUrl = await uploadFileStarGenius(thumbPath, thumbnailFile);
      ...
      const nextLessonNumber = lessons.length > 0
        ? Math.max(...lessons.map((l) => l.lesson_number)) + 1
        : 1;
      ...
      // Actualizar total_lessons del genio
      await supabaseStarGenius
        .from('geniuses')
      ...
      await supabaseStarGenius.from('notifications').insert({
        ...
        message: `"${form.title.trim()}" del genio "${geniusName}" ya esta disponible`,
        data: { lessonId: data.id, geniusId: geniusId, geniusName },
      });
    } catch (error: any) {
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      if (videoFile) {
        const videoPath = generateStoragePathStarGenius('lessons', editingLesson.id, videoFile.name);
        const newVideoUrl = await uploadFileStarGenius(videoPath, videoFile);
        if (!newVideoUrl) throw new Error('No se pudo subir el video');

        // Eliminar video anterior si era de Storage
        if (editingLesson.video_url && isStorageUrlStarGenius(editingLesson.video_url)) {
          await deleteFileByUrlStarGenius(editingLesson.video_url);
        }
      ...
      if (thumbnailFile) {
        const thumbPath = generateStoragePathStarGenius('thumbnails', editingLesson.id, thumbnailFile.name);
        const newThumbUrl = await uploadFileStarGenius(thumbPath, thumbnailFile);
        if (!newThumbUrl) throw new Error('No se pudo subir el thumbnail');

        // Eliminar thumbnail anterior si era de Storage
        if (editingLesson.video_thumbnail_url && isStorageUrlStarGenius(editingLesson.video_thumbnail_url)) {
          await deleteFileByUrlStarGenius(editingLesson.video_thumbnail_url);
        }
    ...
    openConfirm(
      ...
      async () => {
        try {
          ...
          if (lesson.video_url && isStorageUrlStarGenius(lesson.video_url)) {
            await deleteFileByUrlStarGenius(lesson.video_url);
          }
          if (lesson.video_thumbnail_url && isStorageUrlStarGenius(lesson.video_thumbnail_url)) {
            await deleteFileByUrlStarGenius(lesson.video_thumbnail_url);
          }
  ...
  const handleMoveUp = async (index: number) => {
    ...
    try {
      const { error: err1 } = await supabaseStarGenius
        .from('lessons')
      ...
      const { error: err2 } = await supabaseStarGenius
        .from('lessons')
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        <button
          onClick={() => router.push(`/stargenius/genios/${geniusId}`)}
        ...
        <div className="flex-1">
          ...
          <p className="text-surface-500">{geniusName}</p>
        </div>
          ...
          {lessons.map((lesson, index) => (
            <Card key={lesson.id}>
              <div className="flex items-center gap-4 p-4">
                ...
                {lesson.intelligence_type && (
                  <span
                    className={`inline-flex items-center px-2.5 py-1 text-xs font-medium rounded-full shrink-0 ${getIntelligenceBadgeVariant(lesson.intelligence_type)}`}
                  >
                    {getIntelligenceLabel(lesson.intelligence_type)}
                  </span>
                )}
      ...
      >
        <div className="space-y-4">
          <Input
            ...
            placeholder="Ej: El poder de la observacion"
            autoFocus
          />
          ...
          <Textarea
            ...
            placeholder="La frase principal que resume esta leccion..."
            rows={3}
          />
